          ...
          <span key={wordIndex} className="inline-block whitespace-nowrap">
            {word.split("").map((letter) => {
              ...
              const transformedStyles = Object.entries(styles).reduce((acc, [key, value]) => {
                // Cast value to StyleValue<any> to avoid type errors when accessing from/to properties
                const styleValue = value as StyleValue<any>;
                acc[key] = useTransform(proximity, [0, 1], [styleValue.from, styleValue.to]);
                return acc
